...
const items = [
  ...
  getItem('Inventory', 'sub-inventory', <FaBoxes size={20} />, [
    ...
    getItem(<Link href="/stock-movement">Stock Movements</Link>, '/stock-movement'),
  ]),
...
const RootLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <Layout>
      <ModalConfirmSignOut open={open} setOpen={setOpen} handleLogout={handleLogout} isPending={isPending} />
      <Sider
        style={{
          overflow: 'auto',
          height: '100vh',
      ...
      >
        ...
        <Menu theme="dark" mode="inline" selectedKeys={[pathname]} items={items} className="!bg-color-3" />
      </Sider>
      <Layout>
        <Header
          style={{
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <Notification />
            <Button onClick={showModal} icon={<BiLogOutCircle className="-mr-1 text-lg" />} type="primary" className="">
              Log out
            </Button>
            <ThemeToggle />
          </div>
